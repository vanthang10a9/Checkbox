<?php
/**
 * Store Mart Breadcrumb
 */

function zigcy_lite_breadcrumbs( $args = array() ) {

    $breadcrumb = apply_filters( 'zigcy_lite_breadcrumbs_object', null, $args );

    if ( !is_object( $breadcrumb ) )
        $breadcrumb = new zigcy_lite_Breadcrumb_Trail( $args );
 ...
 * @access public
 */
class zigcy_lite_Breadcrumb_Trail {
    ...
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => esc_html__( 'Browse:',                               'zigcy-lite' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'zigcy-lite' ),
            'home'                => esc_html__( 'Home',                                  'zigcy-lite' ),
            'error_404'           => esc_html__( '404 Not Found',                         'zigcy-lite' ),
            'archives'            => esc_html__( 'Archives',                              'zigcy-lite' ),
            // Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
            'search'              => esc_html__( 'Search results for &#8220;%s&#8221;',   'zigcy-lite' ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Page %s',                               'zigcy-lite' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minute %s',                             'zigcy-lite' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Week %s',                               'zigcy-lite' ),
    ...
    protected function add_minute_hour_archive_items() {
        ...
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'zigcy-lite' ) ) );
    }
    ...
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'zigcy-lite' ) ) );
    }
    ...
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'zigcy-lite' ) ) );
    }
    ...
    protected function add_day_archive_items() {
        ...
        $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'zigcy-lite' ) ) );
        $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'zigcy-lite' ) ) );
        $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'zigcy-lite' ) ) );
    ...
    protected function add_week_archive_items() {
        ...
        $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'zigcy-lite' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'zigcy-lite' ) ) );
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'zigcy-lite' ) ) ) );

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ( '%monthnum%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'zigcy-lite' ) ) ) );

                // If using the %day% tag, add a link to the daily archive.
                elseif ( '%day%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'zigcy-lite' ) ) ) );
...
if( ! function_exists('zigcy_lite_header_title_display') ){
    function zigcy_lite_header_title_display(){
        ...
        $zigcy_lite_breadcrumb_image_enable  = get_theme_mod('zigcy_lite_breadcrumb_image_enable','on'); 
        
        if( $zigcy_lite_breadcrumb_image_enable == 'on'  ){
          ?> 
          <div class="store-mart-lite-bread-home ">
            <?php echo wp_kses_post($overlay);?>
            <div class="store-mart-lite-bread-wrapp container">
                ...
                <?php elseif(is_404()) : ?>
                    <h1 class="page-title"> <?php esc_html_e('404 Not Found','zigcy-lite'); ?> </h1>
                <?php elseif(is_category()) : ?>
                    <?php the_archive_title( '<h1 class="page-title">', '</h1>' ); ?>
                <?php elseif(is_search()) : 
                /* translators: %s: search term */?>
                <h1 class="page-title"><?php printf( esc_html__( 'Search Results for: %s', 'zigcy-lite' ), get_search_query() ); ?></h1>
            <?php elseif(is_page()) : ?>
                <?php the_title( '<h1 class="page-title">', '</h1>' ); ?>
        ...
        <?php
        echo '<div class="store-mart-lite-breadcrumb">';
        if(class_exists('woocommerce') && is_woocommerce()){
            $seperator = get_theme_mod('zigcy_lite_breadcrumb_delimeter','/');
            $args = array( 'delimiter' => '<span class="delimiter">'.esc_html($seperator).'</span>' );
            woocommerce_breadcrumb($args);
        }else{
            zigcy_lite_breadcrumbs();  
        } 
        echo '</div>';
        ?>
    </div>
